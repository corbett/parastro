...
template< class Cell_, class Level_ >
class multi_domain_tree
{
public:
	typedef tree<Cell_,Level_> tree_t;
	
	std::vector< tree_t* > m_trees;
	unsigned m_ntrees;
	
	multi_domain_tree( RAMSES::snapshot& snap, const std::vector<int>& mycpus )
	: m_ntrees(0)
	{
		for( unsigned i=0; i<mycpus.size(); ++i )
		{
			m_trees.push_back( new tree_t( snap, mycpus[i], snap.m_header.levelmax ) );//maxlevel , minlevel ) );
			m_trees.back()->read();
			++m_ntrees;
		}
	}
	...
	tree_t& operator[]( size_t i ) const { return *m_trees[i]; } 
	size_t size() const { return m_ntrees; } 
};
